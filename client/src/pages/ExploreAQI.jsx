...
const ExploreAQI = () => {
  ...
  useEffect(() => {
    const getTopCitiesData = async () => {
      ...
      try {
        ...
        setRankingsError(
          "Failed to load top cities AQI data. Please try again later."
        );
      } finally {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      ...
      setSearchError(
        err.message || "Could not find AQI data. Try a different input."
      );
    } finally {
  ...
  const renderAqiDetails = (data) => (
    <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4">
        ...
        <ExportButton aqiData={data} />
      </div>
      <div className="flex items-center space-x-4 mb-4">
        <div
          className={`text-4xl font-bold p-3 rounded-full
          ${
            data.aqi <= 50
              ? "bg-green-500"
              : data.aqi <= 100
      ...
      <h4 className="text-md font-semibold mb-2 text-gray-700 dark:text-gray-300">
        Pollutant Concentrations <span className="notranslate"> (μg/m³)</span>:
      </h4>
        ...
        {Object.entries(data.components || {}).map(
          ([key, value]) =>
            value !== undefined && (
              <div
                key={key}
                className="flex justify-center items-center space-x-2 bg-gray-100 dark:bg-gray-700 w-30 px-4 py-1 rounded-xl"
              >
                <span className="font-medium uppercase notranslate">
                  {key}:
                </span>
        ...
        <ul className="space-y-2">
          {cities.map((city, index) => (
            ...
            >
              ...
              <span
                className={`px-3 py-1 rounded-full text-sm font-semibold w-20 text-center
                ${
                  city.aqi <= 50
  ...
  return (
    <div className="container mx-auto p-6 space-y-8 dark:bg-gray-900 min-h-screen text-gray-900 dark:text-white rounded-3xl">
      <h2 className="text-3xl font-bold text-center text-emerald-700 dark:text-green-500 mb-6">
        ...
        <div className="mt-6">
          ...
          {searchError && !searchLoading && (
            <p className="text-red-600 dark:text-red-400 text-center">
              Map could not load due to search error.
            </p>
          )}
          <MapComponent
